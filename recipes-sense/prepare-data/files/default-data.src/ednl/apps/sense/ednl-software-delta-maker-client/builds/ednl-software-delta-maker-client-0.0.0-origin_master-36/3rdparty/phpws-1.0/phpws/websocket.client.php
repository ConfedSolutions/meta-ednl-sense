...
class WebSocket implements WebSocketObserver
{
	...
	if ($this->socket === false) {
		$this->socket_errno = $errno;
		$this->socket_errstr = $errstr;
		return false;
	} else {
		$this->socket_name_remote = stream_socket_get_name($this->socket, true);
		$this->socket_name_local = stream_socket_get_name($this->socket, false);
	}
        ...
        list($headers,$data) = WebSocketFunctions::parseHeaders($buffer);
	...
	if (strcmp($data,'')!=0) {
		$this->_frames = array_merge($this->_frames, $this->_connection->readFrame($data));
	}
